# CODEMAP — fjolskylduoryggisverndar/openbridge-ci

> File-by-file map of the OpenBridge CI repo: five GitHub Actions workflows (one orchestrator, three reusable per-platform builders, one daily MSIX mirror), plus LICENSE and a leftover .gitignore. Identical to `BuddhaJumpApp/buddhajump-ci` except for the two pieces listed at the end. Every claim cites `file:line`.
...
没有脚本、没有源码。所有构建输入都来自 `PRIVATE_REPO`（应为 `fjolskylduoryggisverndar/openbridge`）指向的私有源码仓库。
...
- **job `check-for-version`**（ubuntu-latest，`:37-59`）
  ...
  - step `get-info`（`:50-59`）：`NAME` ← pubspec `^name:`（本品牌为 `openbridge`）；`PKG` ← `android/app/build.gradle.kts` 的 `applicationId`；`VERSION` ← pubspec `^version:` 整行加 `v`（**保留 `+N`**）。
...
- **外部联系**：私有仓库 `fjolskylduoryggisverndar/openbridge`（读 `pubspec.yaml`、`android/app/build.gradle.kts`）。
...
- **外部联系**：Google Play internal（包名 = `inputs.pkg`）；本仓库 Release；`dl.openbridgeapp.xyz` Worker 通过 `/releases/latest` 间接依赖第 9 步。
