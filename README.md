...
三件套：`fjolskylduoryggisverndar/openbridge`（源码，私有）→ **`BuddhaJumpApp/openbridge-ci`（本仓库，公开）** → `fjolskylduoryggisverndar/openbridge-web`（官网快照，域名 `openbridgeapp.xyz`）。
...
- 发送方必须向 `api.github.com/repos/BuddhaJumpApp/openbridge-ci/dispatches` POST `event_type: pubspec-updated`。
...
gh workflow run build.yml --repo BuddhaJumpApp/openbridge-ci --ref master
...
# 看进度
gh run list --repo BuddhaJumpApp/openbridge-ci --workflow build.yml --limit 5
gh run watch --repo BuddhaJumpApp/openbridge-ci <run-id>
```
...
官网 → `dl.openbridgeapp.xyz/android|windows` → Cloudflare Worker `fjolsky-downloads` → 302 → `edgeone.gh-proxy.org/https://github.com/BuddhaJumpApp/openbridge-ci/releases/download/<tag>/<asset>`。
