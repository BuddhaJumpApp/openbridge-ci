# fjolskylduoryggisverndar/openbridge-ci

> GitHub Actions pipeline for the OpenBridge Flutter VPN client: checks out the private app repo `fjolskylduoryggisverndar/openbridge`, builds Android/iOS/macOS/Windows, publishes the download assets as GitHub Releases **on this repo**, and optionally pushes to Play internal / App Store Connect / TestFlight / Microsoft Store. A byte-for-byte fork of `BuddhaJumpApp/buddhajump-ci` minus two pieces (see §10).
...
| | |
|---|---|
| **是** | OpenBridge 客户端的构建流水线。仓库里只有 5 个 workflow 文件、LICENSE 和一个 `.gitignore`，**没有任何应用源码**。 |
| **是** | 客户端下载产物的发布地：每次构建用 `softprops/action-gh-release` 在**本仓库**建 Release（`build-android.yml:130-138`、`build-windows.yml:148-156`、`build-apple.yml:195-204`）。官网 `openbridgeapp.xyz` 的下载按钮最终指向这里（§7）。 |
| **是** | `BuddhaJumpApp/buddhajump-ci`（母版）的 fork。母版改了 workflow，要手工把同名文件拷过来（§10）。 |
| **不是** | 源码仓库。源码在私有仓库 `fjolskylduoryggisverndar/openbridge`，由 workflow 通过 `PRIVATE_REPO` + `ORG_TOKEN` 检出（`build.yml:44-48`）。改代码、改版本号都要去那边。 |
| **不是** | 自动化的。虽然 `on:` 里写了 `repository_dispatch`，但实际没有可用的发送方（§2），每一次发版都是人手点 `workflow_dispatch`。 |

三件套：`fjolskylduoryggisverndar/openbridge`（源码，私有）→ **`fjolskylduoryggisverndar/openbridge-ci`（本仓库，公开）** → `fjolskylduoryggisverndar/openbridge-web`（官网快照，域名 `openbridgeapp.xyz`）。
...
- 发送方必须向 `api.github.com/repos/fjolskylduoryggisverndar/openbridge-ci/dispatches` POST `event_type: pubspec-updated`。
...
资产命名（`name` = pubspec `name:` = `openbridge`；`version` = 上面的 tag）：

| 平台 | 文件名 | 出处 |
|---|---|---|
| Android APK（universal） | `openbridge-android-universal-<tag>.apk` | `build-android.yml:110` |
| Android AAB（仅 Play 上传失败时） | `openbridge-android-<tag>.aab` | `build-android.yml:102, 140-148` |
| Windows 便携 zip | `openbridge-windows-amd64-<tag>.zip` | `build-windows.yml:144` |
| Windows Store MSIX（仅失败回落） | `openbridge-windows-amd64-<tag>-store.msix` | `build-windows.yml:168` |
| iOS | `openbridge-<tag>.ipa` | `build-apple.yml:148` |
| macOS 未签 pkg 时 | `openbridge-macos-<tag>.zip` | `build-apple.yml:164` |
| macOS TestFlight/Store | `openbridge-<tag>.pkg` | `build-apple.yml:158` |

`openbridge` 不一定等于品牌名：00000vpn/88888vpn 的 pubspec `name:` 是 `vpn00000`/`vpn88888`（Dart 包名不能以数字开头），dl.* Worker 的 `prefix` 字段也照此写（`app-downloads.js:59-66`）。
...
旧版 README 写的 `ANDROID_KEYSTORE_BASE64 / ANDROID_KEYSTORE_PASSWORD / ANDROID_KEY_PASSWORD / ANDROID_KEY_ALIAS` **在任何 yml 里都不存在**，不要照着配。旧 README 里 `PRIVATE_REPO` 的值也已过期（写的是 `BuddhaJumpApp/openbridge`），2026-09-07 起源码仓库在 `fjolskylduoryggisverndar/openbridge`。

| 组 | secret | 用途 | 出处 |
|---|---|---|---|
| 检出 | `PRIVATE_REPO` | 源码仓库 `owner/name`，本仓库应为 `fjolskylduoryggisverndar/openbridge` | 每个 workflow 的 `env:` + `actions/checkout with.repository` |
...
gh workflow run build.yml --repo fjolskylduoryggisverndar/openbridge-ci --ref master
...
# 看进度
gh run list --repo fjolskylduoryggisverndar/openbridge-ci --workflow build.yml --limit 5
gh run watch --repo fjolskylduoryggisverndar/openbridge-ci <run-id>
```

`--repo` 必须是 **ci 仓库**而不是源码仓库。运行前先确认 `fjolskylduoryggisverndar/openbridge` 的 `pubspec.yaml` `version:` 已经改成新版本，否则会往**同一个 tag** 再传一遍（Windows/Apple 那几步 `overwrite_files: true` 会覆盖，Android 那步没有 `overwrite_files`）。
...
官网 → `dl.openbridgeapp.xyz/android|windows` → Cloudflare Worker `fjolsky-downloads` → 302 → `edgeone.gh-proxy.org/https://github.com/fjolskylduoryggisverndar/openbridge-ci/releases/download/<tag>/<asset>`。

Worker 源码 `XTPU/cloudflare/workers/app-downloads.js`：

- `TABLE["openbridgeapp.xyz"]` 一行写着 `repo`、`prefix`、`pin`（`app-downloads.js:22-68`）。**`repo` 必须与本仓库当前的 `owner/name` 一致**。
